...
def get_target_brands(target):
    ...
    div_elements = soup.find_all('div', class_="brands_list p-boxes-3")

    for div_element in div_elements:
        anchor_elements = div_element.find_all('a', class_=lambda x: x.startswith('p-box'))
        hrefs = [a.get('href') for a in anchor_elements]
...
def get_perfume_info(perfumes):
    col_names = ['url','fragrance','brand','perfumer','sex','rating','scent','longevity','sillage','bottle','value_for_money','main_accords','notes','top_notes','heart_notes','base_notes','all_reviews']
    perfume_info = pd.DataFrame(columns=col_names)
    ...
    for i in perfumes:
        ...
        perfume_info.loc[len(perfume_info)] = new_perfume
...
def main():
    all_brands = get_target_brands('https://www.parfumo.com/Popular_Brands')
    brand_name = [i.split('/')[-1] for i in all_brands]

    brand = [i for i in brand_name if i.lower().startswith(sys.argv[1])]
    brand = ['https://www.parfumo.com/Perfumes/'+ i for i in brand]
    all_perfumes = get_perfume_sites(brand)
    output = get_perfume_info(all_perfumes)
    output.to_csv(sys.argv[1]+'.csv',index=False)
